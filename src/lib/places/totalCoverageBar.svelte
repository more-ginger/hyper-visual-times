    ...
    >
        <g class="translate-y-16">
            ...
            {#each visualizationData as country, i}
                <g transform={`translate(${country.rowNumber * fullWidth / 5}, ${fullHeight / 5 + country.columnNumber * fullHeight / 5 + 50})`}>
                    <g>
                        {#each yAxis as tick}
                            {#if tick != 0}
                                <g transform={`translate(0, ${polygonVertexYScale(tick)})`}>
                                    ...
                                    />
                                    <text x="0" y="-5" font-size="10" class="fill-yellow-600">{tick}</text>
                                </g>
                            {/if}
                        {/each}
                    </g>
                    <g transform="translate(20, 0)">
                        <polygon 
                            class="fill-zeit-peach-default"
                            transform="translate(0, 0)"
                            points={`25,${polygonVertexYScale(country.count_zeit)} 0,0 50,0`} 
                            stroke="black"
                        />
                        <polygon 
                            class="fill-nyt-violet-default"
                            transform="translate(50, 0)" points={`25,${polygonVertexYScale(country.count_nyt)} 0,0 50,0`} 
                            stroke="black"
                        />
                    </g>
                    {#if country.max_zeit}
