<script lang="ts">
	...
	const fetchAllArticles = async () => {
		articles = [];
		const articlePromises = nodes.map((country: { country: string; shared_articles: string[] }) =>
...
<div>
	{#await nodes && articles}
		<div>Loading</div>
	{:then articles}
		<div class={`mx-2 my-2 h-100 overflow-auto rounded-md border border-${outlet}-dark`}>
			{#each articles as country, c}
				{#if country.headlines.length > 0}
					<div
						class={`flex w-full ${c !== 0 && 'border-t'} border-${outlet}-dark border-b p-2 text-${outlet}-dark`}
					>
						<div class="flex w-1/2 grow">
					...
					<div class="my-2">
						{#each country.headlines as headline}
							<div class={`border-b border-dotted px-2 border-${outlet}-light text-${outlet}-dark`}>
								{headline}
							</div>
						{/each}
					</div>
