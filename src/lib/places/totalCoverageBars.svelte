<script lang="ts">
	...
	let top20Countries = $derived(
		dataWithoutPrimaryCountry
			.sort((a: parsedCountryData, b: parsedCountryData) => {
				return currentOutlet == 'Zeit' ? b.count_zeit - a.count_zeit : b.count_nyt - a.count_nyt;
			})
			.slice(0, 20)
	);
	...
	const polygonVertexYScale = $derived(
		d3.scaleLinear().domain(d3.extent(domainValues)).range([0, -180])
	);
...
<div class="h-full">
	...
	<div class="h-9/10">
		<svg width="100%" height="100%" bind:clientWidth={fullWidth} bind:clientHeight={fullHeight}>
			<g class="xl:translate-y-8">
				...
				{#each visualizationData as country, i}
					...
					>
						<g class={(showFirstTenOnly && i >= 9) || (showLastTenOnly && i < 10) ? 'hidden' : ''}>
							<g>
								{#each yAxis as tick}
									{#if tick != 0}
										<g transform={`translate(0, ${polygonVertexYScale(tick)})`}>
											...
											/>
											<text x="0" y="-5" font-size="10" class="fill-yellow-600">
												{tick}
											</text>
										</g>
									{/if}
						...
						<TotalCovSvgLabels {country} {step} scale={polygonVertexYScale} />
					</g>
				{/each}
			</g>
		</svg>
