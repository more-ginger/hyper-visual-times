<script lang="ts">
	...
	// data container
	let people: any = $derived(Object.keys(json.data).sort((a,b) => json.data[b].total - json.data[a].total));
	let peopleFlat: any = $derived(people.map((person: any) => {
			let entries = [];
	...
	onMount(() => {
		...
		svg
			...
			.call(d3.axisBottom(x).tickValues([new Date('2024-01-01'), new Date('2024-04-01'), new Date('2024-07-01'), new Date('2024-10-01'), new Date('2024-12-31')]).tickFormat(d => d3.utcFormat("%d. %B")(d)))
	...
	function updateChart() {
		console.log('updating chart',detail,detail[step].includes('donald-trump') ?? false);
		...
		const entered = bubbles
			...
			.on('mouseover', (event, d) => {
				...
				tooltip
					.html(
						`<strong>${d.person}</strong><br>${d3.timeFormat('%Y-%m-%d')(d.week)}<br>${d.count} mentions`
					)
	...
	function togglePerson(event: any) {
		if (event.currentTarget.classList.contains('active')) {
			...
			event.currentTarget.classList.toggle(zeitORnyt == 'NYT' ? 'bg-[var(--color-nyt-light)]' : 'bg-[var(--color-zeit-light)]', false);
		} else {
			event.currentTarget.classList.add('active');
			event.currentTarget.classList.toggle(zeitORnyt == 'NYT' ? 'bg-[var(--color-nyt-light)]' : 'bg-[var(--color-zeit-light)]',true);
		}
		updateChart();
	}
</script>
...
<div class="h-full w-full" bind:clientWidth={width} bind:clientHeight={height}>
	<div id="bubble-chart-controls" class="grid grid-cols-3 gap-1">
		{#each people as person,i}
		<!-- svelte-ignore a11y_no_static_element_interactions, a11y_click_events_have_key_events -->
			<div
				class={"control py bg-[var(--color-ivory-light)] w-[225px] rounded-xl border px-3 hover:cursor-pointer"+ (zeitORnyt == 'NYT' ? ' hover:bg-[var(--color-nyt-light)]' : ' hover:bg-[var(--color-zeit-light)]')}
