<script lang="ts">
	...
	let { articles, currentSource, selectedPerson, selectedDate, x, y } = $props();
...
<div
	class=" absolute h-60 w-[300px] overflow-scroll overscroll-none rounded-xl border border-black backdrop-blur-lg md:h-80 translate-x-[75px] translate-y-[-125px]" style:top={y+"px"} style:left={x+"px"}>
			...
			<div class="pr-2 col-span-2 font-bold">
				...
				<div class="pr-2 col-span-2">
					...
					<img class=" inline" src="icons/ui-forward.svg" alt="arrow right" />
					 <u>{articles.length}</u> Articles 
				</div>
		</div>
	</div>
	<div class="px-2">
		{#each articles as article}
			<div class="bg-[var(--color-ivory-default)] my-3 rounded-xl border border-black p-2">
